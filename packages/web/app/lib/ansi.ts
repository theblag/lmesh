/**
 * Utility to strip ANSI escape codes, window control characters,
 * PowerShell header noise, and terminal repaints from command logs.
 */
export function cleanAnsi(str: string): string {
  ...
  return str
    ...
    .replace(/\x1b[()][AB012]/g, "")
    .replace(/\x1b[=>]/g, "")
    // 2. Remove residual unescaped terminal control sequences (e.g. [?9001h, [2J, [4;1H, [K, ]0;...)
    .replace(/\[\?[0-9]+[hl]/g, "")
    .replace(/\[[0-9;]+[hHlLMK]/g, "")
    .replace(/\[[a-zA-Z]/g, "")
    .replace(/\]0;[^\r\n]*/g, "") // Window title commands
